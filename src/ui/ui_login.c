...
static void
handle_login_button(struct ui_login *ui, enum app_state *state, enum error_code *error, database *user_db, struct user *current_user);
 ...
 */
static void show_and_handle_warning_messages(
    struct ui_login *ui,
...
void ui_login_render(struct ui_base *base, enum app_state *state, enum error_code *error, database *user_db) {
    ...
    show_and_handle_warning_messages(ui, state, error, user_db, ui->current_user);
