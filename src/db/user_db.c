...
int user_db_create_user(database *db, const char *username, bool is_admin, bool reset_password)
{
	...
	const char *sql = "INSERT INTO Users (Username, IsAdmin, ResetPassword, CreatedAt) "
					  "VALUES (?, ?, ?, ?);";
...
int user_db_create_admin(database *db)
{
	...
	const char *sql = "INSERT INTO Users (Username, PasswordHash, Salt, IsAdmin, ResetPassword, CreatedAt) "
					  "VALUES (?, ?, ?, ?, ?, ?);";
...
int user_db_get_by_id(database *db, int user_id, struct user *user_out)
{
	...
	const char *sql = "SELECT UserId, Username, PasswordHash, Salt, IsAdmin, CreatedAt, LastLogin "
...
int user_db_get_by_username(database *db, const char *username, struct user *user_out)
{
	...
	const char *sql = "SELECT UserId, Username, PasswordHash, Salt, IsAdmin, ResetPassword, CreatedAt, LastLogin "
...
int user_db_get_all(database *db)
{
	...
	while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
		...
		printf("| %6d | %-24s | %-5s | %-13s | %-10s |\n", user_id, username, is_admin ? "Yes" : "No", created_at_str,
			   last_login_str);
		printf("+--------+--------------------------+-------+---------------+------------+\n");
	}
