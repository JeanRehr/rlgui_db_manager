...
int user_db_create(database *db, const char *username, const char *password, bool is_admin)
{
	...
	const char *sql = "INSERT INTO Users (Username, PasswordHash, Salt, IsAdmin, CreatedAt) "
					  "VALUES (?, ?, ?, ?, ?);";
...
bool user_db_authenticate(database *db, const char *username, const char *password)
{
    ...
    struct user user = {0};  // Initialize all fields to 0
...
int user_db_update_password(database *db, int user_id, const char *new_password)
{
	...
	const char *sql = "UPDATE Users SET PasswordHash = ?, Salt = ? WHERE UserId = ?;";
...
int user_db_get_all(database *db)
{
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        printf("| %6d | %-24s | %-5s | %-13s | %-10s |\n", 
               user_id, username, is_admin ? "Yes" : "No", created_at_str, last_login_str);
        printf("+--------+--------------------------+-------+---------------+------------+\n");
    }
